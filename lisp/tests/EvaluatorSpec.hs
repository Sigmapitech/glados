  ...
  describe "Evaluator" $ do
    ...
    describe "Variables" $ do
      it "looks up variables in environment" $ do
        let env = extendEnv "x" (VInt 10) emptyEnv
            (result, _) = evalWithEnv (Variable "x") env
        result `shouldBe` Right (VInt 10)

      it "errors on undefined variables" $ do
        let (result, _) = evalWithEnv (Variable "undefined") emptyEnv
    ...
    describe "Define" $ do
      ...
      it "defines with expression" $ do
        let ast = Define "y" (Call (Variable "+") [LiteralInt 2, LiteralInt 3])
            (result, env) = evalWithEnv ast emptyEnv
        result `shouldBe` Right VUnit
    ...
    describe "Lambda" $ do
      it "creates a closure" $ do
        ...
        case result of
          Right (VFunction params _ _) -> params `shouldBe` ["x"]
          _ -> expectationFailure "Expected VFunction"

      it "captures environment" $ do
        let env = extendEnv "y" (VInt 10) emptyEnv
            ast = Lambda ["x"] (Call (Variable "+") [Variable "x", Variable "y"])
            (result, _) = evalWithEnv ast env
        case result of
          Right (VFunction _ _ closureEnv) ->
            lookupEnv "y" closureEnv `shouldBe` Just (VInt 10)
          _ -> expectationFailure "Expected VFunction"

    describe "Function Calls" $ do
      it "calls a lambda" $ do
        let lambda = Lambda ["x"] (Call (Variable "+") [Variable "x", LiteralInt 1])
            ast = Call lambda [LiteralInt 5]
            (result, _) = evalWithEnv ast emptyEnv
        result `shouldBe` Right (VInt 6)

      it "errors on argument count mismatch" $ do
        let lambda = Lambda ["x", "y"] (Variable "x")
            ast = Call lambda [LiteralInt 5]
            (result, _) = evalWithEnv ast emptyEnv
        result `shouldSatisfy` isLeft

      it "calls nested lambdas" $ do
        let outer = Lambda ["x"] (Lambda ["y"] (Call (Variable "+") [Variable "x", Variable "y"]))
    ...
    describe "Built-in Arithmetic" $ do
      it "adds integers" $ do
        let ast = Call (Variable "+") [LiteralInt 1, LiteralInt 2, LiteralInt 3]
            (result, _) = evalWithEnv ast emptyEnv
        result `shouldBe` Right (VInt 6)

      it "subtracts integers" $ do
        let ast = Call (Variable "-") [LiteralInt 10, LiteralInt 3, LiteralInt 2]
            (result, _) = evalWithEnv ast emptyEnv
        result `shouldBe` Right (VInt 5)

      it "multiplies integers" $ do
        let ast = Call (Variable "*") [LiteralInt 2, LiteralInt 3, LiteralInt 4]
            (result, _) = evalWithEnv ast emptyEnv
        result `shouldBe` Right (VInt 24)

      it "divides integers" $ do
        let ast = Call (Variable "div") [LiteralInt 10, LiteralInt 2]
            (result, _) = evalWithEnv ast emptyEnv
        result `shouldBe` Right (VInt 5)

      it "errors on division by zero" $ do
        let ast = Call (Variable "div") [LiteralInt 10, LiteralInt 0]
            (result, _) = evalWithEnv ast emptyEnv
        result `shouldSatisfy` isLeft

      it "calculates modulo" $ do
        let ast = Call (Variable "mod") [LiteralInt 10, LiteralInt 3]
            (result, _) = evalWithEnv ast emptyEnv
        result `shouldBe` Right (VInt 1)

    describe "Built-in Comparison" $ do
      it "checks equality for integers" $ do
        let ast1 = Call (Variable "eq?") [LiteralInt 5, LiteralInt 5]
            ast2 = Call (Variable "eq?") [LiteralInt 5, LiteralInt 3]
      ...
      it "checks less-than" $ do
        let ast1 = Call (Variable "<") [LiteralInt 3, LiteralInt 5]
            ast2 = Call (Variable "<") [LiteralInt 5, LiteralInt 3]
            (result1, _) = evalWithEnv ast1 emptyEnv
            (result2, _) = evalWithEnv ast2 emptyEnv
        result1 `shouldBe` Right (VBool True)
        result2 `shouldBe` Right (VBool False)
    ...
    describe "Conditionals" $ do
      ...
      it "evaluates condition expression" $ do
        let ast = If (Call (Variable "<") [LiteralInt 3, LiteralInt 5]) (LiteralInt 10) (LiteralInt 20)
            (result, _) = evalWithEnv ast emptyEnv
        result `shouldBe` Right (VInt 10)
    ...
    describe "Complex Expressions" $ do
      it "evaluates nested arithmetic" $ do
        let ast = Call (Variable "+") [LiteralInt 1, Call (Variable "*") [LiteralInt 2, LiteralInt 3]]
            (result, _) = evalWithEnv ast emptyEnv
        result `shouldBe` Right (VInt 7)

      it "evaluates lambda with computation" $ do
        let lambda = Lambda ["x"] (Call (Variable "*") [Variable "x", Variable "x"])
            ast = Call lambda [LiteralInt 5]
            (result, _) = evalWithEnv ast emptyEnv
      ...
      it "evaluates factorial-like expression" $ do
        let ast =
              If
                (Call (Variable "eq?") [LiteralInt 0, LiteralInt 0])
                (LiteralInt 1)
                (Call (Variable "*") [LiteralInt 5, LiteralInt 4])
            (result, _) = evalWithEnv ast emptyEnv
        result `shouldBe` Right (VInt 1)

      it "evaluates define followed by use" $ do
        let defineAst = Define "double" (Lambda ["x"] (Call (Variable "*") [Variable "x", LiteralInt 2]))
            (_, env1) = evalWithEnv defineAst emptyEnv
            callAst = Call (Variable "double") [LiteralInt 7]
