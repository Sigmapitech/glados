...
sexprSpec = do
  describe "SExpr" $ do
    ...
    it "should create SSymbol" $ do
      SSymbol (SymbolName "x") `shouldBe` SSymbol (SymbolName "x")
      SSymbol "+" `shouldBe` SSymbol (SymbolName "+")
...
astSpec = do
  describe "Ast" $ do
    ...
    it "should create Variable" $ do
      Variable (VarName "x") `shouldBe` Variable (VarName "x")
      Variable "y" `shouldBe` Variable (VarName "y")

    it "should create Define" $ do
      Define (VarName "x") (LiteralInt 5) `shouldBe` Define "x" (LiteralInt 5)

    it "should create Lambda" $ do
      let lambda = Lambda [ParamName "x", ParamName "y"] (Variable "x")
      lambda `shouldBe` Lambda ["x", "y"] (Variable "x")

    it "should create Call" $ do
      let call = Call (Variable "+") [LiteralInt 1, LiteralInt 2]
      call `shouldBe` Call (Variable (VarName "+")) [LiteralInt 1, LiteralInt 2]
      ...
      let nested =
            Define
              ...
              ( Lambda
                  ["n"]
                  ( If
                      (Call (Variable "eq?") [Variable "n", LiteralInt 0])
                      (LiteralInt 1)
                      (Call (Variable "*") [Variable "n", Call (Variable "factorial") [Call (Variable "-") [Variable "n", LiteralInt 1]]])
                  )
              )
      nested `shouldBe` nested

-- | Test suite for Value data type
valueSpec :: Spec
valueSpec = do
  describe "Value" $ do
    it "should create VInt" $ do
      VInt 42 `shouldBe` VInt 42
      VInt (-1) `shouldBe` VInt (-1)

    it "should create VBool" $ do
      VBool True `shouldBe` VBool True
      VBool False `shouldBe` VBool False

    it "should create VUnit" $ do
      VUnit `shouldBe` VUnit

    it "should compare VInt values" $ do
      VInt 1 == VInt 1 `shouldBe` True
      VInt 1 == VInt 2 `shouldBe` False

    it "should compare VBool values" $ do
      VBool True == VBool True `shouldBe` True
      VBool True == VBool False `shouldBe` False

    it "should compare VUnit values" $ do
      VUnit == VUnit `shouldBe` True

    it "should not compare VFunction values as equal" $ do
      let f1 = VFunction ["x"] (Variable "x") []
      let f2 = VFunction ["x"] (Variable "x") []
      f1 == f2 `shouldBe` False

    it "should not compare different Value constructors as equal" $ do
      VInt 1 == VBool True `shouldBe` False
      VBool True == VUnit `shouldBe` False
      VInt 0 == VUnit `shouldBe` False
...
evaluatorSpec = do
  ...
  describe "throwEvalError" $ do
    it "should create an error result" $ do
      let computation = throwEvalError "test error" :: Evaluator Value
      let (result, _) = runEvaluator computation emptyEnv
...
errorSpec = do
  ...
  describe "liftError" $ do
    ...
    it "should add context to Left values" $ do
      (liftError "parsing" (Left "syntax error") :: Either ErrorMsg Int) `shouldBe` Left (ErrorMsg "parsing: syntax error")
